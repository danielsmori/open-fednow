 ...
 *
 * <p>The RTP gateway is a documented stub: TCH certificate validation and
 * XML envelope parsing are not yet implemented, but the routing contract —
 * that inbound messages are forwarded to {@link MessageRouter} and the
 * response is returned unmodified — is fully testable and must hold.
 *
 * <p>No Spring context is loaded. {@link MessageRouter} and
 * {@link CertificateManager} are mocked directly.
 *
 * <h2>Coverage</h2>
 * <ul>
 *   <li>/rtp/health — returns 200 with the stub status message</li>
 *   <li>/rtp/receive — routes through {@link MessageRouter#routeInbound} for ACSC</li>
 *   <li>/rtp/receive — propagates RJCT response unmodified (Layer 1 must not alter pacs.002)</li>
 *   <li>/rtp/receive — does not invoke {@link CertificateManager} (TCH validation is pending)</li>
 * </ul>
 */
class RtpGatewayTest {
    ...
    void setUp() {
        ...
        gateway = new RtpGateway(messageRouter, certificateManager);
    }
    ...
    @Test
    void receiveTransfer_routesInboundToMessageRouter() {
        Pacs008Message inbound = buildMessage("E2E-RTP-001", "TXN-RTP-001", "250.00");
        Pacs002Message acsc = Pacs002Message.accepted("E2E-RTP-001", "TXN-RTP-001");
        when(messageRouter.routeInbound(inbound)).thenReturn(ResponseEntity.ok(acsc));

        ResponseEntity<Pacs002Message> actual = gateway.receiveTransfer(inbound);
        ...
        verify(messageRouter).routeInbound(inbound);
    }

    @Test
    void receiveTransfer_returnsMessageRouterResponseUnmodified() {
        // Layer 1 must not transform the pacs.002 produced by Layers 2–4;
        // the same contract holds for both FedNow and RTP gateways.
        Pacs008Message inbound = buildMessage("E2E-RTP-002", "TXN-RTP-002", "500.00");
        Pacs002Message rjct = Pacs002Message.rejected(
                "E2E-RTP-002", "TXN-RTP-002", "AM04", "Insufficient funds");
        when(messageRouter.routeInbound(inbound)).thenReturn(ResponseEntity.ok(rjct));
    ...
    @Test
    void receiveTransfer_doesNotInvokeCertificateManager() {
        ...
        Pacs008Message inbound = buildMessage("E2E-RTP-003", "TXN-RTP-003", "100.00");
        when(messageRouter.routeInbound(inbound)).thenReturn(
                ResponseEntity.ok(Pacs002Message.accepted("E2E-RTP-003", "TXN-RTP-003")));

        gateway.receiveTransfer(inbound);
